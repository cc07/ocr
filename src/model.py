...
class CtcOcr:
    ...
    def _default_conv_layer(self, inputs, n_filters, kernel_size, w_init, b_init, batch_norm=True, activation=tf.nn.elu, padding='SAME'):
    ...
    def _build_inception_a_layer(self, inputs, n_filters, w_init, b_init):
        with tf.variable_scope('conv1'):
            conv1 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)

        with tf.variable_scope('conv2'):
            conv2 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)
            conv2 = self._default_conv_layer(conv2, n_filters, (1, 3), w_init, b_init)
            conv2 = self._default_conv_layer(conv2, n_filters, (3, 1), w_init, b_init)

        with tf.variable_scope('conv3'):
            conv3 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)
            conv3 = self._default_conv_layer(conv3, n_filters, (1, 5), w_init, b_init)
            conv3 = self._default_conv_layer(conv3, n_filters, (5, 1), w_init, b_init)
            conv3 = self._default_conv_layer(conv3, n_filters, (1, 5), w_init, b_init)
            conv3 = self._default_conv_layer(conv3, n_filters, (5, 1), w_init, b_init)

        with tf.variable_scope('conv4'):
            conv4 = self._default_max_pool(inputs, pool_size=(3, 3), strides=(1, 1))
            conv4 = self._default_conv_layer(conv4, n_filters, (1, 1), w_init, b_init)

        with tf.variable_scope('concat'):
            inception = tf.concat([conv1, conv2, conv3, conv4], axis=3)
            inception = self._default_conv_layer(inception, n_filters, (1, 1), w_init, b_init, batch_norm=True, activation=None)
            inception = inception + inputs

        return inception

    def _build_reduction_a_layer(self, inputs, n_filters, w_init, b_init):
        with tf.variable_scope('conv1'):
            conv1 = self._default_conv_layer(inputs, n_filters, (3, 3), w_init, b_init)

        with tf.variable_scope('conv2'):
            conv2 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)
            conv2 = self._default_conv_layer(conv2, n_filters, (3, 3), w_init, b_init)
            conv2 = self._default_conv_layer(conv2, n_filters, (3, 3), w_init, b_init)
        ...
        with tf.variable_scope('concat'):
            ...
            reduction = self._default_conv_layer(reduction, n_filters, (1, 1), w_init, b_init, batch_norm=True, activation=None)

        return reduction

    def _build_inception_b_layer(self, inputs, n_filters, w_init, b_init):
        with tf.variable_scope('conv1'):
            conv1 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)

        with tf.variable_scope('conv2'):
            conv2 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)
            conv2 = self._default_conv_layer(conv2, n_filters, (1, 7), w_init, b_init)
            conv2 = self._default_conv_layer(conv2, n_filters, (7, 1), w_init, b_init)
    ...
    def _build_reduction_b_layer(self, inputs, n_filters, w_init, b_init):
        with tf.variable_scope('conv1'):
            conv1 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)
            conv1 = self._default_conv_layer(conv1, n_filters, (3, 3), w_init, b_init)
        ...
        with tf.variable_scope('conv4'):
            conv4 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)
            conv4 = self._default_conv_layer(conv4, n_filters, (3, 3), w_init, b_init)
    ...
    def _build_inception_c_layer(self, inputs, n_filters, w_init, b_init):
        with tf.variable_scope('conv1'):
            conv1 = self._default_conv_layer(inputs, n_filters, (1, 1), w_init, b_init)
    ...
    def _build_stem(self, inputs, n_filters, w_init, b_init):
        stem = self._default_conv_layer(inputs, n_filters, (3, 3), w_init, b_init)
        stem = self._default_conv_layer(stem, n_filters, (3, 3), w_init, b_init, batch_norm=True)
        stem = self._default_max_pool(stem, strides=(3, 2))
        stem = self._default_conv_layer(stem, n_filters, (3, 3), w_init, b_init)
        stem = self._default_conv_layer(stem, n_filters, (3, 3), w_init, b_init, batch_norm=True)
        stem = self._default_max_pool(stem, strides=(2, 2))
    ...
    def _build_conv_layer(self, inputs):
        ...
        with tf.variable_scope('stem'):
            stem = self._build_stem(inputs, self.n_filters, w_init, b_init)
        ...
        with tf.variable_scope('inception_a'):
            ...
            for i in range(5):
                with tf.variable_scope('inception_a_{}'.format(i)):
                    inception_a = self._build_inception_a_layer(inception_a, self.n_filters, w_init, b_init)
            inception_a = self._default_max_pool(inception_a, strides=(2, 2))

        with tf.variable_scope('reduction_a'):
            reduction_a = self._build_reduction_a_layer(inception_a, self.n_filters, w_init, b_init)
            reduction_a = self._default_max_pool(reduction_a, strides=(1, 1))
        ...
        with tf.variable_scope('inception_b'):
            ...
            for i in range(10):
                with tf.variable_scope('inception_b_{}'.format(i)):
                    inception_b = self._build_inception_b_layer(inception_b, self.n_filters, w_init, b_init)
            inception_b = self._default_max_pool(inception_b, strides=(1, 1))

        with tf.variable_scope('reduction_b'):
            reduction_b = self._build_reduction_b_layer(inception_b, self.n_filters, w_init, b_init)
            reduction_b = self._default_max_pool(reduction_b, strides=(1, 1))
        ...
        with tf.variable_scope('reduction_c'):
            ...
            for i in range(5):
                with tf.variable_scope('inception_c_{}'.format(i)):
                    inception_c = self._build_inception_c_layer(inception_c, self.n_filters, w_init, b_init)
            inception_c = self._default_max_pool(inception_c, strides=(1, 1))
    ...
    def _build_rnn_cells(self):
        ...
        for _ in range(self.n_rnn_cells):
            cell = tf.contrib.rnn.LayerNormBasicLSTMCell(self.n_units)
            cells.append(cell)
    ...
    def _build_rnn_layer(self, inputs, seq_len, seed=7):
        ...
        with tf.variable_scope('rnn_fw'):
            cell_fw = self._build_rnn_cells()

        with tf.variable_scope('rnn_bw'):
            cell_bw = self._build_rnn_cells()
